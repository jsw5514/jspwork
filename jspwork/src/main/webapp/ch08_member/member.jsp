...
<script src="${pageContext.request.contextPath}/ch08_member/script.js"></script>    
<%--    다음 우편번호 api 호출--%>
    ...
    function findAddr(){
        new daum.Postcode({
            oncomplete: function(data) {
                ...
                if(data.userSelectedType=='R') { //사용자가 선택한 주소가 도로명주소인 경우
                    if(data.bname != ''){
                    ...
                    let roadAddress = data.roadAddress + (extraAddr != '' ? '('+extraAddr+')' : '');
                    document.getElementById('addr').value = roadAddress;
                }
                else{//사용자가 선택한 주소가 지번주소인 경우
...
<body>
	<div>
		<form id="frm" method="post" action="memberProc.jsp">
			<table class="table">
				<tr>
				...
				<tr>
					...
					<td>
						<input name="id">
						<input type="button" value="ID중복확인" onclick="idCheck(document.querySelector('[name=&quot;id&quot;]').value, '${pageContext.request.contextPath}')">
				...
				<tr>
					<td colspan="3" align="center">
						<input type="button" value="회원가입" onclick="inputCheck('${pageContext.request.contextPath}')">&emsp;&emsp;
